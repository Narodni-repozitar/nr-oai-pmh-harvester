...
# TODO: dořešit testy pro taxonomie
...
def test_find_in_title_jsonb(app, db):
    ...
    terms = query.all()
    assert len(terms) > 0
    assert str(query) == """SELECT taxonomy_term.id AS taxonomy_term_id, taxonomy_term.slug AS 
    taxonomy_term_slug, taxonomy_term.extra_data AS taxonomy_term_extra_data, taxonomy_term.level 
    AS taxonomy_term_level, taxonomy_term.parent_id AS taxonomy_term_parent_id, 
    taxonomy_term.taxonomy_id AS taxonomy_term_taxonomy_id, taxonomy_term.taxonomy_code AS 
    taxonomy_term_taxonomy_code, taxonomy_term.busy_count AS taxonomy_term_busy_count, 
    taxonomy_term.obsoleted_by_id AS taxonomy_term_obsoleted_by_id, taxonomy_term.status AS 
    taxonomy_term_status 
FROM taxonomy_term JOIN taxonomy_taxonomy ON taxonomy_taxonomy.id = taxonomy_term.taxonomy_id 
WHERE taxonomy_taxonomy.code = %(code_1)s AND taxonomy_term.status = %(status_1)s AND (
taxonomy_term.extra_data @> %(extra_data_1)s) ORDER BY taxonomy_term.slug"""
